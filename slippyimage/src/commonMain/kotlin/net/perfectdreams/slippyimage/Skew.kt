...
//***************************************************************************
//**  net.perfectdreams.slippyimage.Skew Class
//***************************************************************************
/**
 ...
 *
 * Ported to SlippyImage (Kotlin Multiplatform) by MrPowerGamerBR
 */
class Skew(private val src: SlippyImage) {
    protected var edgeAction = ZERO
    protected var interpolation = NEAREST_NEIGHBOUR
    ...
    val originX: Float
        get() = x0 - min(min(x0, x1), min(x2, x3)).toInt()

    val originY: Float
        get() = y0 - min(min(y0, y1), min(y2, y3)).toInt()

    fun setCorners(
        targetImage: SlippyImage,
        x0: Float,
        y0: Float,
        x1: Float,
        y1: Float,
    ...
    private fun filter(src: SlippyImage, dst: SlippyImage): SlippyImage {
        val dst = dst
        val width = src.width
    ...
    protected fun filterPixelsNN(dst: SlippyImage, width: Int, height: Int, inPixels: IntArray, transformedSpace: Rectangle): SlippyImage {
        val outWidth = transformedSpace.width
        val outHeight = transformedSpace.height
        val outX: Int
        val outY: Int
     ...
     */
    fun getRGB(image: SlippyImage, x: Int, y: Int, width: Int, height: Int): IntArray {
        return image.getRGB(x, y, width, height)
    }
     ...
     */
    fun setRGB(image: SlippyImage, x: Int, y: Int, width: Int, height: Int, pixels: IntArray) {
        var i = 0
        for (targetY in y until y + height) {
            for (targetX in x until x + width) {
                image.setRGB(targetX, targetY, pixels[i++])
            }
        }
    }
